...
/**********BEGIN NEWCODE******************/
/* initialzing a permenent use time count list */
static BufferUseTime BufferUseTimeCountList[1000];
/**************END NEWCODE***************/
			...
			bool *foundPtr)
{
	...
	*foundPtr = FALSE;
	...
	/**************************BEGIN NEWCODE*******************/
	FoundOldOrNot = FALSE;
	for (i = 0; i < sizeof(BufferUseTimeCountList)/sizeof(BufferUseTime); i ++)
	{
		if (BufferUseTimeCountList[i].blockNum == buf->tag.blockNum)
		{
			FoundOldOrNot = TRUE;
			break;
		}
	...
	/* If we can find the buffer in list */
	if (FoundOldOrNot)
	{
		BufferUseTimeCountList[i].usetimecount = buf->usetimecount;
	}
	/* Initialize the use time for new buffer*/
	else
	{
		BufferUseTime newBuffer;
		newBuffer.blockNum = buf->tag.blockNum;
		newBuffer.usetimecount = buf->usetimecount;
		if (sizeof(BufferUseTimeCountList)/sizeof(BufferUseTime) < 1000)
		{
			BufferUseTimeCountList[sizeof(BufferUseTimeCountList)/sizeof(BufferUseTime)] = newBuffer;
		}
	}

	/*update usetime for each buffer*/
	buf->usetimecount = 1;
	for (i = 0; i < sizeof(BufferUseTimeCountList)/sizeof(BufferUseTime); i ++)
	{
		if (BufferUseTimeCountList[i].blockNum == buf->tag.blockNum)
		{
			buf->usetimecount = BufferUseTimeCountList[i].usetimecount + 1;
			break;
		}
	}
	/****************************END NEWCODE*********************/
