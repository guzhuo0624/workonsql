...
AddBufferToFreelist(BufferDesc *bf)
{
	...
	/**********BEGIN NEWCODE******************/
	/* find the right position to insert the buffer*/
	current = &(BufferDescriptors[SharedFreeList->freePrev]);
	while (current->buf_id != Head && current->usetimecount > bf->usetimecount)
	{
		current = &(BufferDescriptors[current->freePrev]);
	}
