...
/*******BEGIN NEWCODE*******/
static int ExecEstimateHash(double est_rows);
static int ExecEstimateHashFunctions(int bitsize, double est_rows);
static unsigned int FNVHash(uint32 key);
static unsigned int SDBMHash(uint32 key);
static unsigned int DEKHash(uint32 key);
static unsigned int JSHash(uint32 key);
static unsigned int RSHash(uint32 key);
static unsigned int GetHash(int i, uint32 key);
static int GetBit(BloomBitArray bitarray, int n);
static void SetBit(BloomBitArray bitarray, int n);
static int bitsize;
static int est_r;
/***********END NEWCODE********/
...
ExecHash(HashState *node)
{
	...
	for (;;)
	{
		...
		/*******BEIGN NEWCODE*******/
		ExecBloomChangebit(bitarray, econtext, hashkeys);
		/********END NEWCODE********/
...
/************BEGIN NEWCODE***********/
/*
	it creates a bit array for Bloom filter to use
	if palloc false, then return null
*/
BloomBitArray
ExecBloomCreate(double est_rows)
{
	BloomBitArray bitarray;
	est_r = EST_R;
	if (est_rows < est_r){
	...
	bitsize = ExecEstimateHash(est_rows);
	est_r = est_rows;
	printf("\n\
				bit size:%d\n", bitsize);
	est_r = est_rows;
	if(!(bitarray= (BloomBitArray) palloc0(bitsize/sizeof(char)))) 
	{
		return NULL;
	}
...
/*
	it destroy the bitarray of bloom filter
*/
void
ExecBloomDestroy(BloomBitArray bitarray)
{
	pfree(bitarray);
}

/*BOTH ExecBloomChangebit and ExecMembership are inspaired by
  function called ExecHashGetBucket, it basiclly find the bucket by
  corresponding hashkey, which is what we need, we need to find the hash value
  of bit array and set it to 1, or get the value from the hash.
 */
/*
	it chenges the corresponding bit to be 1 in the bitarray
*/

void
ExecBloomChangebit(BloomBitArray bitarray,
					   ExprContext *econtext,
					   List *hashkeys)
{
	List	   *hk;
	...
	foreach(hk, hashkeys)
	{
		...
		if (!isNull)
		{	
			num_hash = ExecEstimateHashFunctions(bitsize,est_r);
			if (num_hash > max_hash){
				num_hash = max_hash;
			}
			for(i = 0; i <= num_hash; i++)
			{
				int r = GetHash(i, intKey) % bitsize;
				SetBit(bitarray, r);
			}
		}
...
/*
	it return true or false if the hasheky is in bitarray or not
*/
bool
ExecMembership(BloomBitArray bitarray,
			 	   ExprContext *econtext,
				   List *hashkeys)
{
	List	   *hk;
	...
	foreach(hk, hashkeys)
	{
		...
		if (!isNull)
		{
			...
			{
				int r = GetHash(i, intKey) % bitsize;
				inornot = inornot && GetBit(bitarray, r);
			}
		}
...
/*	
	set bit to be 1 in bit array
*/
static void
SetBit(BloomBitArray bitarray, int n)
{
	(bitarray[(int) n/sizeof(char)] |= (1<<(n & 7)));
}

/*
	get bit from the corresponding position
*/
static int
GetBit(BloomBitArray bitarray, int n)
{
	int result = (bitarray[(int) n/sizeof(char)] & (1<<(n & 7)));
	return result;
}
...
static int
ExecEstimateHash(double est_rows)
{
	double false_positive = FALSE_POSITIVE;
	double bitarraysize = 0;

	bitarraysize = ((- log(false_positive) * est_rows) / pow(log(2), 2));

	return (int)bitarraysize;
}

static int
ExecEstimateHashFunctions(int bitarraysize, double est_rows)
{
	int num_hash = 0;

	num_hash = ((bitarraysize/est_rows) * log(2));

	return (int)num_hash;
}

static unsigned int
GetHash(int i, uint32 key)
{
	unsigned int hash;
	if (i == 0) {
		hash = FNVHash(key);
