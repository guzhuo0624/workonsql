...
ExecHashJoin(HashJoinState *node)
{
	...
	if (!node->hj_hashdone)
	{
		...
		/***********BEGIN NEWCODE*****/
		bitarray = ExecBloomCreate(outerNode->plan->plan_rows);
		node->hj_bitarray = bitarray;
		Total_positive = 0;
		True_positive = 0;
		True_negative = 0;


		hashNode->bitarray = bitarray;
		/**********END NEWCODE*******/
	...
	for (;;)
	{
		...
		if (node->hj_NeedNewOuter)
		{
			...
			/************BEGIN NEWCODE************/
			if (!ExecMembership(bitarray, econtext, outerkeys))
			{
				True_negative++;
				node->hj_NeedNewOuter = true;
				continue;
			...
			if (ExecQual(joinqual, econtext, false))
			{
				...
				if (otherqual == NIL || ExecQual(otherqual, econtext, false))
				{
					...
					if (isDone != ExprEndResult)
					{
						...
						/*******BEGIN NEWCODE********/
						/*this is if we find the correct value for hash*/
						True_positive++;
						/**********END NEWCODE********/
						return result;
					}
...
ExecEndHashJoin(HashJoinState *node)
{
	...
	if (node->hj_bitarray)
	{
		int False_positive = Total_positive - True_positive;
		int Total_Drops = False_positive + True_negative;
		printf("\n\
				Bloom Filter result:\n\
				Total Positives: %d\n\
				True Positives: %d\n\
				...
				\n",
				Total_positive, True_positive, False_positive, True_negative, Total_Drops, False_positive, Total_Drops);
		ExecBloomDestroy(node->hj_bitarray);
		node->hj_bitarray = NULL;
	}
	/*********END NEWCODE**************/
	/*
